  ...
  # 
  # raw <- RSocrata::read.socrata("https://data.austintexas.gov/resource/7d8e-dm7r.csv?month=4&vehicle_type=scooter",
  ...
  # write_rds(raw, "02_raw_data/austin_mm_april_scooters.rds")
  
  raw <- readr::read_rds("02_raw_data/austin_mm_april_scooters.rds")
  ...
  length(unique(raw$device_id))# 17725 unique scooters, though not a useful var to analyze
  
  table(raw$vehicle_type) # all scooters (as per query); can drop var from data
  table(raw$month)
  table(raw$year) #2018, 2019, and 2020!
  
  length(unique(raw$council_district_start)) #14 unique council districts
  length(unique(raw$council_district_end)) #14 unique council districts
  ...
  length(unique(raw$census_geoid_start)) #169 unique 2010 census tracks
  length(unique(raw$census_geoid_end)) #173 unique 2010 census tracks
  ...
  prop.table(table(raw$trip_distance < 0))*100#less than 1% thankfully
